...
class Data:
    ...
    FOOD = colours.RED
    SNAKE_PRIMARY = colours.BLUE1
    SNAKE_SECONDARY = colours.BLUE2
...
class SnakeGameAI:
    ...
    def _update_ui(self):
        self.display.fill(data.BACKGROUND)
        
        for pt in self.snake:
            pygame.draw.rect(self.display, data.SNAKE_PRIMARY, pygame.Rect(pt.x, pt.y, BLOCK_SIZE, BLOCK_SIZE))
            pygame.draw.rect(self.display, data.SNAKE_SECONDARY, pygame.Rect(pt.x+4, pt.y+4, 12, 12))
            
        pygame.draw.rect(self.display, data.FOOD, pygame.Rect(self.food.x, self.food.y, BLOCK_SIZE, BLOCK_SIZE))
        
        text = font.render("Score: " + str(self.score), True, data.FONT_COLOUR)
